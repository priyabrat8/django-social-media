...
def index(request):
    ...
    {'user_profile':user_profile,'posts':feed_list,'user':request.user,'suggestion_username_profile_list':suggestion_username_profile_list[:4], 'suggestion_username_profile_list_length':len(suggestion_username_profile_list)}
    )
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                new_porfile = Profile.objects.create(user=user_model,id_user=user_model.id)
                new_porfile.save()
